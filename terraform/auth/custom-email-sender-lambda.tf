module "custom-email-sender-lambda" {
  source      = "./custom-email-sender-lambda"
  prefix      = var.prefix
  aws_profile = var.aws_profile

  app_url = "https://forgedandfound.co.uk" # TODO: fix this
  cognito_domain            = local.cognito_domain
  cognito_email_kms_key_arn = aws_kms_key.cognito_email.arn
